...
static void create_directory_name(DGACallback *c, struct tm timestamp) {
  ...
  strcat(dir_name, "/");
  if (ttwatch_get_watch_name(c->watch, dir_name + strlen(dir_name), sizeof(dir_name) - strlen(dir_name))
      ==TTWATCH_NoError)
    strcat(dir_name, "/");
...
void do_get_activities(TTWATCH *watch, OPTIONS *options, uint32_t formats) {
  ...
  if (ttwatch_enumerate_files(watch, TTWATCH_FILE_TTBIN_DATA, do_get_activities_callback, &dgacallback)
      !=TTWATCH_NoError)
...
static void do_get_activity_summaries_callback(uint32_t id, uint32_t length, void *cbdata) {
  ...
  if (protobuf)
    ...
    sprintf(filename,
            "Summary_%08X-%02d-%02d-%02d.protobuf",
            id,
...
void do_get_activity_summaries(TTWATCH *watch, OPTIONS *options, uint32_t formats) {
  DGACallback dgacallback = {watch, options, formats};
  if (ttwatch_enumerate_files(watch, TTWATCH_FILE_ACTIVITY_SUMMARY, do_get_activity_summaries_callback, &dgacallback)
      !=TTWATCH_NoError)
    write_log(1, "Unable to enumerate files\n");
}
