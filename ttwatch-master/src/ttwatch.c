...
static int decode_activity_character(char ch, int *activity) {
  switch (ch) {
    case 'r': *activity = ACTIVITY_RUNNING;
      break;
    case 'c': *activity = ACTIVITY_CYCLING;
      break;
    case 's': *activity = ACTIVITY_SWIMMING;
      break;
    case 't': *activity = ACTIVITY_TREADMILL;
      break;
    case 'f': *activity = ACTIVITY_FREESTYLE;
      break;
    default:return 0;
  }
  return 1;
...
void do_create_continuous_race(TTWATCH *watch, char *race) {
  ...
  while (*race) {
    ...
    *ptr = 0;
    ...
    cbdata.race_data_file =
        (TTWATCH_RACE_HISTORY_DATA_FILE *) realloc(cbdata.race_data_file, cbdata.race_data_file_length);
...
static void do_list_history_callback(TTWATCH_ACTIVITY activity,
                                     int index,
                                     const TTWATCH_HISTORY_ENTRY *entry,
                                     void *data) {
...
void do_delete_history_item(TTWATCH *watch, const char *item) {
  ...
  switch (item[0]) {
    case 'r': activity = TTWATCH_Running;
      break;
    case 'c': activity = TTWATCH_Cycling;
      break;
    case 's': activity = TTWATCH_Swimming;
      break;
    case 't': activity = TTWATCH_Treadmill;
      break;
    case 'f': activity = TTWATCH_Freestyle;
      break;
    case 'g': activity = TTWATCH_Gym;
      break;
    default:write_log(1, "Invalid activity type specified, must be one of r, c, s, t, f or g\n");
      return;
  }
...
void do_display_settings(TTWATCH *watch) {
  ...
  for (i = 0; i < defn_count; ++i) {
    ...
    switch (definitions[i]->type) {
      case MANIFEST_TYPE_ENUM:enum_defn = (struct MANIFEST_ENUM_DEFINITION *) definitions[i];
        ...
        break;
      case MANIFEST_TYPE_INT:int_defn = (struct MANIFEST_INT_DEFINITION *) definitions[i];
        write_log(0, "%d %s", (int) value, int_defn->units);
        break;
      case MANIFEST_TYPE_FLOAT:float_defn = (struct MANIFEST_FLOAT_DEFINITION *) definitions[i];
...
void help(char *argv[]) {
  ...
  write_log(0,
            "  -a, --auto                 Same as \"--update-fw --update-gps --get-activities --get-summaries --set-time\"\n");
