...
void export_tcx(TTBIN_FILE *ttbin, FILE *file) {
  ...
  switch (ttbin->activity) {
    ...
    case ACTIVITY_SWIMMING: fputs("Pool Swim", file);
      break;
    case ACTIVITY_TREADMILL: fputs("Running", file);
      break; /* per Garmin spec, not "Treadmill" */
    case ACTIVITY_FREESTYLE: fputs("Freestyle", file);
      break;
    case ACTIVITY_INDOOR: fputs("Biking", file);
      break;
    case ACTIVITY_GYM: fputs("Gym", file);
      break;
    ...
    switch (record->tag) {
      case TAG_TRAINING_SETUP:
        switch (record->training_setup.type) {
          case TRAINING_LAPS_TIME: lap.trigger_method = "Time";
            break;
          case TRAINING_LAPS_DISTANCE: lap.trigger_method = "Distance";
            break;
        }
        break;
      ...
      case TAG_CYCLING_CADENCE:cc_sensor_packet(&cc_data, &record->cycling_cadence);
        break;
      case TAG_INTERVAL_SETUP:break;
      case TAG_INTERVAL_START:break;
      case TAG_INTERVAL_FINISH:
        ...
        break;
      case TAG_LAP:lap.time = record->lap.total_time - lap_start_time;
        if (ttbin->activity==ACTIVITY_TREADMILL) {
