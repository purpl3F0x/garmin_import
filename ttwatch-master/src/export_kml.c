...
void export_kml(TTBIN_FILE *ttbin, FILE *file) {
  ...
  switch (ttbin->activity) {
    ...
    case ACTIVITY_CYCLING: fputs("Cycling", file);
      break;
    case ACTIVITY_SWIMMING: fputs("Swimming", file);
      break;
  ...
  switch (ttbin->activity) {
    case ACTIVITY_RUNNING: type_text = "RUNNING";
      break;
    case ACTIVITY_CYCLING: type_text = "CYCLING";
      break;
    case ACTIVITY_SWIMMING: type_text = "SWIMMING";
      break;
    case ACTIVITY_TREADMILL: type_text = "TREADMILL";
      break;
    case ACTIVITY_FREESTYLE: type_text = "FREESTYLE";
      break;
    default: type_text = "UNKNOWN";
      break;
  }
  ...
  for (i = 0; i < ttbin->gps_records.count; ++i) {
    if ((ttbin->gps_records.records[i]->gps.timestamp!=0) &&
      ...
      fprintf(file, "                <gx:coord>%.6f %.6f %d</gx:coord>\r\n",
              ...
              isnan(ttbin->gps_records.records[i]->gps.elevation) ? 0
  ...
  for (i = 0; i < ttbin->gps_records.count; ++i) {
    if ((ttbin->gps_records.records[i]->gps.timestamp!=0) &&
      ...
      fprintf(file, "                            <gx:value>%.*f</gx:value>\r\n",
              (ttbin->gps_records.records[i]->gps.cum_distance==0.0f) ? 0 : (5
                  - (int) floor(log10(ttbin->gps_records.records[i]->gps.cum_distance))),
