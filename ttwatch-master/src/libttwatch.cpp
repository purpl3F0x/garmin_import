...
int ttwatch_open_device(libusb_device *device, const char *serial_or_name, TTWATCH **watch) {
  ...
  *watch = (TTWATCH *) calloc(1, sizeof(TTWATCH));
  ...
  count = libusb_get_string_descriptor_ascii(handle,
                                             desc.iSerialNumber,
                                             (unsigned char *) (*watch)->serial_number,
                                             sizeof((*watch)->serial_number));
  if (count > 0)
...
int ttwatch_get_serial_number(TTWATCH *watch, char *serial, size_t max_length) {
  ...
  size_t count =
      libusb_get_string_descriptor_ascii(watch->device, desc.iSerialNumber, (unsigned char *) serial, max_length);
  if (count < max_length)
    serial[count] = 0;
...
int ttwatch_clear_data(TTWATCH *watch) {
  ...
  foreach (it, files)
  {
    ...
    switch (it->first & TTWATCH_FILE_TYPE_MASK) {
      ...
      case TTWATCH_FILE_HISTORY_DATA:RETURN_ERROR(ttwatch_delete_file(watch, it->first));
        break;

      case TTWATCH_FILE_HISTORY_SUMMARY:RETURN_ERROR(ttwatch_read_whole_file(watch,
                                                                             it->first,
                                                                             (void **) &history,
                                                                             &length));
        {
