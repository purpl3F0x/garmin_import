...
int main(int argc, char *argv[]) {
  ...
  if (truncate) {
    switch (truncate_mode) {
      case TRUNCATE_LAPS: truncate_laps(ttbin);
        break;
      case TRUNCATE_RACE: truncate_race(ttbin);
        break;
      case TRUNCATE_GOAL: truncate_goal(ttbin);
        break;
      case TRUNCATE_INTERVAL: truncate_intervals(ttbin);
        break;
      default:
        truncate_laps(ttbin) || truncate_intervals(ttbin) ||
            truncate_race(ttbin) || truncate_goal(ttbin);
        break;
    }
  }
